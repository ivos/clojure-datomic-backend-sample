...
(defn user-create
  ...
  (let [conn (:connection request)
        ...
        tx (conj tx
                 ...
                 [:ensure-unique nil :user/email (:user/email data)])
...
(defn user-update
  ...
  (let [conn (:connection request)
        ...
        tx (conj tx
                 ...
                 [:ensure-unique eid :user/email (:user/email data)])
        tx-result @(d/transact conn tx)
        _ (log/trace "Tx result" tx-result)
        db-after (:db-after tx-result)
        saved (get-entity db-after eid)
...
(defn user-delete
  ...
  (let [conn (:connection request)
        ...
        delete-sessions-tx (->> (get (d/entity db eid) :session/_user)
                             (map :db/id)
                             (map #(vector :db.fn/retractEntity %)))
        tx (vec (concat tx delete-sessions-tx))
        tx-result @(d/transact conn tx)
        _ (log/trace "Tx result" tx-result)
        db-after (:db-after tx-result)
